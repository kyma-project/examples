...
function usage {
    ...
    echo "               unlock:      Solve package-lock problems."
    ...
    echo "               prod:        Build with production configuration."
...
if [[ ! -z $INSTALL_PEER ]]; then
    ENV+="-e INSTALL_PEER "
fi

# Run in the buildpack container
echo "🚀 Running make $GOAL on $CONTAINER_PATH:"
docker run --rm -v $HOST_PATH:$CONTAINER_PATH -w $CONTAINER_PATH $ENV eu.gcr.io/kyma-project/node-buildpack:0.0.8 /bin/bash -c "make $GOAL"
