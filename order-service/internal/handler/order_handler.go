...
func (o *Order) RegisterAll(root string, router Router) {
	...
	router.HandleFunc(fmt.Sprintf("%s/{id}", root), o.OnDelete).Methods(http.MethodDelete)
}
...
func (o *Order) OnDelete(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	vars := mux.Vars(r)
	err := o.svc.Delete(ctx, vars["id"])
	if err == store.NotFoundError {
		log.Println(err)
		w.WriteHeader(http.StatusNotFound)
		return
