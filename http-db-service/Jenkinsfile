...
def application = 'http-db-service'
                ...
                timeout(time:20, unit:"MINUTES") {
                    ansiColor('xterm') {
                        ...
                        stage("push image $application") {
                            sh "docker tag ${application}:latest ${dockerPushRoot}example/${application}:${dockerImageTag}"
                            sh "docker push ${dockerPushRoot}example/${application}:${dockerImageTag}"
                        }
                    }
                }
            }
